...
robots.txt는 주요 검색엔진 크롤러의 접근을 허용하고, 검색엔진이 sitemap.xml, feed.xml, rss.xml 위치를 쉽게 확인할 수 있도록 구성했습니다.
...
sitemap.xml은 현재 실제 존재하는 메인 1개 및 서브 8개 URL만 포함합니다.
...
feed.xml과 rss.xml은 검색엔진 및 외부 수집기가 사이트의 주요 페이지 구조를 더 쉽게 이해할 수 있도록 메인·서브 페이지별 제목, 링크, 설명, 카테고리 정보를 포함합니다.
...
- robots.txt에서 sitemap 위치가 정상 노출되어야 함
...
- sitemap.xml은 검색엔진에 URL 발견을 돕는 파일이지, 상위 노출을 보장하는 파일이 아닙니다.
- robots.txt에서 허용하더라도 검색엔진이 모든 페이지를 반드시 색인하는 것은 아닙니다.
